    ...
    <main class="container mx-auto px-6 pt-24 pb-12">
        <div class="bg-white rounded-xl shadow-lg p-6 card animate-fade-in">
            <div class="flex justify-between items-center mb-6">
                ...
                <x-button href="{{ route('addPakan', ['username' => $username]) }}">
            ...
            <div class="overflow-x-auto">
                <table class="w-full bg-white rounded-lg">
                    ...
                    <tbody>
                        @forelse ($pakan as $row)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="p-4 font-medium">{{ $row['nama_pakan'] }}</td>
                                <td class="p-4">{{ $row['jenis'] }}</td>
                                <td class="p-4">{{ $row['stok'] }}</td>
                                <td class="p-4 flex space-x-2">
                                    <x-button href="{{ route('editPakan', ['id' => $row['id'], 'username' => $username]) }}" class="bg-yellow-600 hover:bg-yellow-700">
                                        <i class="fas fa-edit mr-1"></i>Edit
                                    </x-button>
                                    <x-button href="{{ route('deletePakan', ['id' => $row['id'], 'username' => $username]) }}" class="bg-red-600 hover:bg-red-700">
